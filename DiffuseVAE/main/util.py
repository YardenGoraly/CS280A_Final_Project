...
def get_dataset(name, root, image_size, norm=True, flip=False, **kwargs):
    ...
    if name == "celeba":
        ...
        # TODO: use SOCS dataset, fix parameters
        dataset = SOCSDataset(root, norm=False, transform=None)
    elif name == 'movi-recons':
        assert image_size == 128
        # TODO: use SOCS dataset, fix parameters
        dataset = SOCSReconDataset(root, norm=False, transform=None)
    elif name == 'movi-recons-single':
        assert image_size == 128
        # TODO: use SOCS dataset, fix parameters
        dataset = SOCSReconSingleDataset(root, norm=False, transform=None)
    else:
        raise NotImplementedError(
