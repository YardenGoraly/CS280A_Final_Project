...
class DDPMWrapper(pl.LightningModule):
    ...
    def forward(
        ...
        if self.resample_strategy == "spaced":
            ...
            indices = space_timesteps(num_steps, num_steps, type=self.skip_strategy)
            # import pdb
            # pdb.set_trace()
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.conditional:
            x = batch
            with torch.no_grad():
                # NOTE: VAE reconstruction, input x is x_0
                mu, logvar = self.vae.encode(x * 0.5 + 0.5)
                z = self.vae.reparameterize(mu, logvar)
                cond = self.vae.decode(z)
                cond = 2 * cond - 1
                # NOTE: replacement for conditioning, 
                # but if we don't want conditioning then... else block
                cond = self.vae(x) # TODO: does this work?
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        if not self.conditional:
            ...
            # import numpy as np
            # t = x_t.cpu().squeeze().permute(1, 2, 0).detach().numpy() * 255.
            # Image.fromarray(t.astype(np.uint8)).save(f'/home/leohuang/data/base/temp_{batch_idx}.png')
            # import pdb
            # pdb.set_trace()
            return self(
                x_t,
